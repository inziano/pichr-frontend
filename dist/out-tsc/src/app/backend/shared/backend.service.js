var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Injectable } from '@angular/core';
import { Http } from '@angular/http';
// import 'rxjs/add/operator/map';
import { map } from 'rxjs/operators';
// Service imports
import { EndpointsService } from '../../core/shared/endpoint.service';
var BackendService = /** @class */ (function () {
    // Constructor
    function BackendService(http, endpoint) {
        this.http = http;
        this.endpoint = endpoint;
        // Users endpoint
        this.users = this.endpoint.fullUrl + 'user/all';
    }
    // Get all users
    BackendService.prototype.getAllUsers = function () {
        // Hit api and get all users
        return this.http.get(this.users, this.endpoint.options).pipe(map(function (response) {
            return response.json();
        }));
    };
    BackendService = __decorate([
        Injectable(),
        __metadata("design:paramtypes", [Http, EndpointsService])
    ], BackendService);
    return BackendService;
}());
export { BackendService };
//# sourceMappingURL=backend.service.js.map